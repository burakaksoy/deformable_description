...
def create_dlo_urdf_equal_segment_length(length=1,
                                         radius=0.01,
    ...
    urdf_obj = Robot(
        ...
        add_prismatic_joints(prefix=model_name, limits=environment_limits_xyz),
        
        add_revolute_joints_n_cylinders(prefix=model_name, 
        ...
        add_holding_points_n_center(prefix=model_name,
                                    num_segments=simplified_dlo_num_segments,
                                    segment_l=segment_length,
    ...
    urdf_str, is_valid_urdf = _validate_and_visualize_urdf(str(urdf_obj), visualize=visualize)
    
    return urdf_str, is_valid_urdf

def prism_joint(N,robot_name,origin,axis_str,limits_meter, max_effort=1, max_velocity=1.0):
    N = int(N)
    ret = Joint(
...
def add_prismatic_joints(prefix, limits):
    global N_global  # Declare N_global as global to modify the global variable inside this function
    ...
    i = 0
    for axis in axes:
        link = Link(name=prefix+"_link_"+str(N_global))
        joint = prism_joint(N_global, prefix, [0, 0, 0], axis, limits[2*i:2*i+2], prism_joint_effort, prism_joint_max_velocity)
...
def rev_joint(N,robot_name,origin,axis_str,limits_deg, max_effort=100, max_velocity=1.0):
    N = int(N)
    ret = Joint(
        Parent(link=robot_name+"_link_"+str(N-1)),
        Child(link=robot_name+"_link_"+str(N)),
        Origin(origin),
        Axis(xyz=axis_str),
        Limit(lower=limits_deg[0]*PI/180, upper=limits_deg[1]*PI/180, effort=max_effort, velocity=max_velocity),
        type="revolute",
        name= robot_name+"_rev_joint_"+str(N))
    return ret

def cylindrical_link(N,robot_name,material,r,l):
    """
...
def add_revolute_joints_n_cylinders(prefix, 
                                    limits, 
                                    num_segments, 
                                    segment_r, 
                                    segment_l, 
    ...
    for n in range(num_segments):
        ...
        if n == 0:
            ...
            joint = rev_joint(N_global, prefix, [0, 0, 0], axes[i], [-180, 180], rev_joint_effort, rev_joint_max_velocity)
            ...
            link = cylindrical_link(N_global, prefix, "Teal", segment_r, segment_l)
            ...
            joint = rev_joint(N_global, prefix, [0, 0, segment_l], axes[i], limits[2*i:2*i+2], rev_joint_effort, rev_joint_max_velocity)
            ...
            joint = rev_joint(N_global, prefix, [0, 0, 0], axes[i], limits[2*i:2*i+2], rev_joint_effort, rev_joint_max_velocity)
        ...
        if n == num_segments - 1:
            ...
            joint = Joint(Parent(link=prefix+"_link_"+str(N_global-1)),
                          Child(link=prefix+"_link_"+str(N_global)),
...
def add_holding_points_n_center(prefix, 
                                num_segments, 
                                segment_l, 
...
def _validate_and_visualize_urdf(urdf_str, visualize=False):
    is_valid_urdf = False
    urdf_model = None
    try:    
...
def test_dlo_urdf_creator_equal_segment_length(visualize=False):
    ...
    urdf_str, is_valid_urdf = create_dlo_urdf_equal_segment_length(visualize=visualize)
